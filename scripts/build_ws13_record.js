...
module.exports = {
  ...
  sections: [
    ...
    { type: 'callout', text: 'REVISED TWICE on 2026-08-12, and the second revision reverses the first. Morning: filed with the Friday-open fill RECOMMENDED, not adopted. Afternoon: the owner adopted it. Evening: reversed to the Friday CLOSING auction. The reversal came from two findings, neither of which was in the original work. First, widening the cost stress from 2x to 4x put the break-even at 1.82x - the open fill stops being an improvement once executing there costs about 80 per cent more than the close, which a thinner opening auction can plausibly exceed, so “costs nothing measurable” was too strong. Second, and larger: the entire justification rested on a Singapore operator being unable to trade a 04:00 SGT close. A market-on-close order is submitted during the session and executes in the closing auction unattended, so the operational objection had a standard order type as its answer and the premise was wrong. The close also keeps the executed price identical to the modelled one, which the open would have permanently forfeited since the engines were deliberately not rebuilt. The refresh cadence decided alongside the first revision STANDS: Friday morning, operator-run, so the instruction precedes both auctions.' },
    ...
    { type: 'table',
      ...
      rows: [
        ...
        ['Fill at the Friday close, market-on-close', 'ADOPTED 2026-08-12', 'Deepest and best-discovered auction of the day; no spread crossed; and it is the price the record is computed on, so published figures and fills are the same number'],
        ['Fill at the Friday open', 'ADOPTED then REVERSED, same day', `${s4(B.FRI.open_minus_close.sharpe)} Sharpe with the interval straddling zero; break-even at ~1.82x the assumed cost, which a thinner opening auction can plausibly exceed. The 04:00 SGT objection it solved is answered by a market-on-close order instead`],
