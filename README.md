...
453 pytest tests across 36 files. Key suites:
...
- `test_incremental_fetch.py` (19 tests) — incremental fetch ≡ full fetch on identical underlying data (payloads equal ex `fetched_at_utc`), negative-cache decide/seed/cap/merge behaviour, EDGAR still consulted when the primary attempt is negative-cache skipped (the SOXX guarantee).
...
**Incremental constituent fetch (2026-07-27)**: `refresh_all.py` fetches constituents incrementally by default. The 2026-07-26 scheduled run measured 232.4 minutes end-to-end, of which `fetch_constituents` was 202.8 minutes (87%) — dominated not by cold caches but by permanently-missing Fridays re-attempted through the iShares anti-bot retry ladder every single week (anti-bot HTML is correctly never cached, so each such Friday burns the ~45s backoff ladder per run: ICHN 67.0m with its 82 pre-launch/2019 holes, IUCM 33.5m, NDIA 20.3m, SOXX 9.6m, and even hole-free ETFs ~3–5m each on CSV re-parsing plus the head-Friday ladder). `compute_breadth` (13.8m) and Steps 2–6 (15.7m) were never the problem. Incremental mode reuses every Friday that already has a REAL iShares-derived snapshot in the committed `constituents_*.json` (no network, no raw-cache dependency), and records known-missing Fridays in `data/fetch_negative_cache.json` (committed): each is re-probed with a single no-backoff request at most every 30 days, capped at 16 probes per ETF-run, while Fridays younger than 30 days are always attempted with the full ladder (recent gaps are usually iShares publication lag). Carried-forward and EDGAR-sourced snapshots are never reused — those Fridays re-resolve live each run, so carry-forward chains and the SOXX EDGAR freshness comparison behave exactly as in a full run. A registry change (URL or ticker_overrides) auto-falls back to a full re-fetch; parse-rule changes otherwise require `refresh_all.py --full` or `scripts/regenerate_constituents_from_cache.py`. Guard evidence (2026-07-27, per the unattended-agent rule): `scripts/verify_incremental_parity.py` ran full-vs-incremental for CSP1 (iShares-UK), EXV1 (Xetra) and ICHN (stress case) and found constituents and breadth outputs EXACTLY equal apart from the wall-clock stamps (fetch full→incremental: CSP1 316s→55s, EXV1 206s→52s, ICHN 4,261s→53s — the residual ~50 incremental seconds are the live retry ladder on that week's still-unpublished Friday file; on a Saturday run with the file published this collapses to seconds); `tests/test_incremental_fetch.py` pins the same invariant offline, including EDGAR-consulted-despite-skip. Weekly deltas to `fetch_negative_cache.json` in scheduled commits are normal bookkeeping; delete an entry (or the file) to force an immediate re-attempt. CI is unaffected — it never runs Step 1.
