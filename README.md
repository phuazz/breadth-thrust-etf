...
The local refresh and daily valuation publisher rebuild both pages. The component-email workflow sends a sealed snapshot and does not rebuild either page. `test_simple_page_parity.py` enforces that published holdings, sleeve split and headline statistics are derivable from `live_track.json` and `risk_overlay.json`, positions and prices share one as-of date, and disclosures are present.
...
|---|---|
| Xetra: Fri 23:30 (CEST) / Sat 00:30 (CET); NYSE: Sat 04:00 (EDT) / 05:00 (EST) | Normal-session Friday closing times, expressed in SGT. This is the information the decision reads. |
...
| After each verified component refresh | The orchestrator builds and seals the proposed book automatically, with each venue's decision session named. Do not replace a sealed component instruction with an ad-hoc legacy rebuild. |
| Before the broker-confirmed deadline for each fill session | Confirm the venue and order-type submission deadline with the broker; consult the dashboard's **Execution Timing** tab. Do not use a closing-auction time as an order deadline. |
| Xetra: Mon 23:30 (CEST) / Tue 00:30 (CET); NYSE: Tue 04:00 (EDT) / 05:00 (EST) | Normal-session Monday closing times in SGT, not broker cutoffs or trade authorisation. Use the per-sleeve fill date in `live_targets.json`. |
...
CEST/CET and EDT/EST label the venue's daylight-saving/standard-time regime; every clock time in the table is SGT. Europe and the US change clocks on different dates. The table assumes normal sessions and an ordinary Friday/Monday week: holidays can move the decision/fill dates, and early closes change the times. The checker derives closing times from each venue's calendar; consult the [Xetra trading calendar](https://www.xetra.com/xetra-en/trading/trading-calendar-and-trading-hours) and [NYSE trading information](https://www.nyse.com/trade/trading-information) for exchange schedules, and confirm broker deadlines separately.
...
**Both tasks remain armed (`--push`).** Weekend refreshes now publish the verified core (A/B/C, EM tilt and breadth gate) before attempting Europe. D retains its existing selection when its required-session data is incomplete; a separately verified portfolio-risk adjustment may resize that held basket. No Thursday-close substitute or redistribution of D's budget is permitted. A failed Europe attempt cannot roll back an already committed core. Post-fill runs retain the existing full-refresh path.

**Two-stage factsheet delivery.** The first verified core snapshot triggers an initial email to every existing recipient. The consolidated email follows when D is ready, or at the Sunday 18:00 SGT review target with D explicitly on HOLD. If all strategies are ready at the first send, only the consolidated email is needed. One D-ready follow-up is permitted through Monday 06:00 SGT; later changes need operator review. These are review targets, not broker cutoffs or guaranteed delivery times. GitHub cron can be delayed. The Sunday 17:00 SGT check remains a reconciliation wake-up.

The sender uses `data/component_release.json` and the source files in the Git commit that created that seal, so a subsequent daily valuation update does not change the emailed book. It does not rerun engines. Headline model performance is labelled separately from proposed trades; readable HTML and JSON attachments contain the complete proposed book. Both stages use `RECIPIENT_EMAIL`; technical alerts use only `GMAIL_USER`. `docs/component_delivery.json` records an attempt **before** SMTP and confirms it afterwards. An interrupted or partially accepted send blocks retries until reconciled. The existing `release_factsheet.py --hold` veto remains authoritative. Manual workflow dispatch defaults to a no-send rehearsal and cannot bypass validation. See [component delivery runbook](handoffs/component-sender-deployment.md).
...
**The same split exists on the US side (2026-09-09).** SPY and XLF carried the 2026-09-08 close from 20:38 UTC and kept it, while LIN, CRH, SW and AMCR — foreign-domiciled names in the IUMS roster — were unserved at 01:24 UTC when the panel was built and all four carried it by 06:42 UTC. IUMS duly capped at 2026-09-04 and `live_targets` reported sleeve A HOLD on 13 of its 14 panels. Norgate does not cover for that group: under `--price-source auto` their columns stay on the incumbent because the Norgate series is not a date superset of the cached one (measured on IUMS: AMCR, CRH, DD, LIN and SW, each starting later at Norgate after a corporate action), so the lag is a property of the refresh hour rather than of the source. LIN is on the probe from 2026-09-09 to measure the settle hour, `--summary` now reports currency by firing hour as well as on average, and `check_vendor_probe.py` keeps that one line out of the retraction email until 2026-09-23 — an exemption that expires by itself — so a fortnight of measurement is not spent emailing the cycle being measured. SPY and XLF are not exempt.
