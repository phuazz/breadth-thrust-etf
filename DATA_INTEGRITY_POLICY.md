...
|------|----------|-----------|
| **OK** | ≥ 90% | Coverage printed and recorded in `data_quality.roster_coverage_latest` |
| **Warning** | 50–90% | Warning to stdout; panel still written |
| **Fail** | < 50% | Loud stderr banner; **`compute_breadth.py` exits 2 and does NOT write**, leaving the previous panel in place |

Floors first calibrated against all 38 committed panels on 2026-08-09, with WARN at 0.85. Healthy sits at 97–100% (30 of 38 at exactly 100%). That calibration read a structural tail at ITWN 89.7% and ICHN 93.6%; the ITWN reading turned out to be a resolver bug — an unmapped "Gretai Securities Market" venue dropping seven Taipei Exchange names into the roster as unpriceable bare codes — and with the venue mapped ITWN runs at 98.7%, so on 2026-08-16 WARN was raised to 0.90 at the owner's instruction. The structural tail is now ICHN alone at 93.6%, where some Chinese lines genuinely lack yfinance history. WARN sits below that tail so a structurally-lower panel does not warn every week and teach the operator to ignore it; FAIL sits below anything a real roster has produced. The raise cost nothing in false alarms: replayed over the last 252 trading days across all 24 guarded panels, 0.90 blocks exactly the same 26 days and the same 6 of 50 Fridays as 0.85, because the breaches that occur are deep dips rather than marginal ones — nothing observed sits in the [0.85, 0.90) band on a day the guard reads. Panels written between 2026-08-09 and the raise stamp `roster_coverage_warn_floor: 0.85` in their `data_quality` block; that is correct provenance (the floor in force when they were written), not drift.
...
**The WARN band is blocked at commit time, not at write time.** Writing a thin panel is tolerable because the alternative is a stale one; *committing* one is not — a 61.5% panel reached main on 2026-08-08 and changed Strategy A's holdings. So `check_refresh_guard.py` gained **G6**, which FAILS the pre-commit guard for any deployed panel below the 90% WARN floor, importing the floor from `compute_breadth` so there is one definition. It prefers `data_quality.roster_coverage_latest` and falls back to deriving `n_with_ma50 / n_constituents` from the series, because every panel written before 2026-08-09 lacks the recorded field and a check that skipped 23 of 24 panels would be worse than none. An unreadable panel warns rather than fails: absence of evidence is not evidence of thinness.

### 5e. Coverage-depth guard on the refresh (added 2026-09-02)

Sections 5c and 5d watch the **tail** of a panel: whether the cache advanced, and whether the current roster was priced. Neither can see a panel whose **history** has thinned, because a delisted name is not in today's roster, and the tail is exactly where a survivor-basis rebuild looks healthy.

That is what the 2026-09-02 post-fill refresh did. It ran from the automation clone, whose gitignored `data/prices_cache_*.parquet` files had never received the WS11 (2026-08-10) and WS16 (2026-08-13) Norgate delisted-archive backfills held only in the main tree's caches, nor the Norgate columns the 2026-08-31 hand run wrote into them. All fifteen US-constituent panels were rebuilt on the survivor basis — XLNX and MXIM absent from SOXX, SIVB and FRC from CSP1 and IUFS, TWTR and ATVI from IUCM — and 2018 coverage (roster-days priced over roster-days in the roster) fell on every one: SOXX 0.9997 → 0.8193, IUCM 0.9978 → 0.5385, IUES 1.0000 → 0.6282. Sleeve A's headline Sharpe rose 0.9196 → 0.9623 and the ungated blend 1.1864 → 1.2011, an unsanctioned restatement upward that reached the dashboard at `62292ed` with the refresh guard, the price-panel guard and 1,752 tests all green. Record: `reviews/2026-08-30_ws19_norgate-constituent-prices.md`, section 13.

`scripts/check_coverage_depth.py` is the sixth VERIFY step in `refresh_all.py`. Two checks:

| Check | Reads | Verdict |
|-------|-------|---------|
| **C1 coverage depth** | every US panel's `series` against `data/coverage_baseline.json` | Per calendar year, `sum(n_with_price) / sum(n_constituents)`, measured over the baseline's own date window. A panel-year more than **0.01** below its baseline **FAILS**; a baseline year with no sessions **FAILS**; a panel-year more than 0.01 above **WARNS** — a restatement to file, since a rise cannot be the survivor mechanism. |
| **C2 delisted probes** | `data/prices_cache_<etf>.parquet` for SOXX (XLNX, MXIM), CSP1 and IUFS (SIVB, FRC), IUCM (TWTR, ATVI) | A probe column that is absent or empty in a cache that exists **FAILS**. An absent cache (gitignored — a runner, a worktree) **SKIPS**. |

C2 is deliberately redundant with C1 on a healthy tree. It is the guard on the guard — a baseline regenerated by mistake from a survivor tree would pass C1 while every probe was empty — and it sees a loss C1 cannot: one name's column in a 500-name panel moves yearly coverage by about 0.002.

**Scope: the fifteen US-constituent panels**, selected by the registry rule "no exchange suffix" (`check_coverage_depth.us_panels`), not by trading calendar — the four country panels share the NYSE calendar but never held what the survivor cache lost, and were unchanged to four decimals on 2026-09-02. Fifteen rather than fourteen because IUIT, pruned from sleeve A's universe on 2026-05-23, is still deployed and regressed identically.

**The baseline is committed and carries its provenance** — the commit it was measured on, when, and why — and is written only by hand: `python scripts/check_coverage_depth.py --write-baseline --ref <commit> --why "..."`, which refuses to overwrite without `--force`. Re-baselining is the sign-off act for a sanctioned restatement; `refresh_all.py` never calls the writer. The first baseline is the panels committed at `670ca1c` (2026-08-31), the last filed basis. Adopting `--price-source auto` for the panels would be such a restatement and takes a re-baseline with its sign-off.

**Tolerance 0.01, calibrated.** Across the five commits that rewrote the panels between 2026-08-15 and 2026-08-31 on the same basis, closed years were identical to four decimal places on all fifteen panels and the open year drifted at most 0.0021 (IUCS 2026) as the tail filled in; the smallest first-year fall in the incident was 0.0350 (IUUS 2018, one delisted name in a 31-name panel). The tolerance sits five times above the worst healthy drift and three and a half times below the smallest regression, and both bounds are pinned by the tests.

**Like for like.** The current panel is measured over the baseline's `start_date..end_date`, so the open year compares the sessions both sides hold rather than a year one side has since extended. Sessions beyond the baseline's end are the tail, which Sections 5c and 5d cover.

**On a FAIL** nothing is committed or pushed (`scheduled_refresh.py` exits 3 and emails). Restore the tree's `data/prices_cache_*.parquet` from a tree that carries the backfills, re-run `compute_breadth.py` for the failing panels, re-run the refresh. If the change is a sanctioned restatement, re-baseline with `--force` and file the sign-off with it.

Guarded by `tests/test_check_coverage_depth.py`: the pure logic including both date-window boundaries, both tolerance bounds against the measured numbers, the committed baseline's provenance and scope, the 2018 depth of the filed basis (so the file cannot be quietly regenerated from a survivor tree), the wiring into `refresh_all.py`, and — for panels built after the baseline was adopted — the committed panels themselves.
